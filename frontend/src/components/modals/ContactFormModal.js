...
const isValidPhone = (str) => {
    ...
    // Allow digits, spaces, hyphens, parens, dots, plus sign
    if (!/^[+]?[\d\s\-().]{7,20}$/.test(s))
...
export default function ContactFormModal({contact, onClose, onSave}) {
    ...
    const updateEmail = (i, key, val) => {
        ...
        setField('emails', arr);
        if (key === 'value') {
            // Clear just the error for this specific email row
            setErrors(prev => {
    ...
    const updatePhone = (i, key, val) => {
        ...
        setField('phones', arr);
        if (key === 'value') {
            // Clear just the error for this specific phone row
            setErrors(prev => {
    ...
    const validate = () => {
        ...
        if (!fnCheck.ok) {
            newErrors.firstName = fnCheck.msg;
            valid = false;
        }
        ...
        if (form.lastName.trim()) {
            ...
            if (!lnCheck.ok) {
                newErrors.lastName = lnCheck.msg;
                valid = false;
            }
        }
        ...
        if (!titleCheck.ok) {
            newErrors.title = titleCheck.msg;
            valid = false;
        }
        ...
        // At least one email or phone must be filled in
        ...
        form.emails.forEach((e, i) => {
            ...
            if (!val) return;
            if (emailSet.has(val)) {
                newErrors.emails[i] = 'Duplicate email address.';
                valid = false;
            }
            emailSet.add(val);
        });
        ...
        form.phones.forEach((p, i) => {
            ...
            if (!val) return;
            if (phoneSet.has(val)) {
                newErrors.phones[i] = 'Duplicate phone number.';
                valid = false;
            }
            phoneSet.add(val);
        });
